---
description: Cut a full release — bump the build number ONCE, then upload to TestFlight, upload to the Play internal and closed tracks, and build the Android APK off that same number
---

# /release — cut a full iOS + Android release

Ship the current `main` to **both** stores off a **single** build-number bump:
upload to TestFlight (iOS), upload an AAB to the Play `internal` and `alpha`
tracks (Android OTA), and produce a signed release APK (Android sideload). This is
...
## 6. Android — build the AAB and upload to the Play testing tracks
...
- Run `tools/upload_play.sh` — uploads the bundle and rolls it out to every
  track in `PLAY_TRACKS` (default `internal,alpha`) in a single edit. `internal`
  is live immediately; `alpha` (the closed track wired to the tester Google
  Group) goes through Google review first. A 401/403 right after first-time
  setup is usually permission propagation — wait and retry.
...
- **Android (Play)**: the new versionCode is on both tracks — live now on
  `internal` (no processing delay), and queued for Google review on `alpha`, so
  Google Group testers see it only once that clears. Confirm with
  `tools/check_play_status.sh`, which reports both. If step 6 was skipped (no
  service account), say so.
