...
- **Scripts (Godot view layer, test-excluded):**
  ...
  - `HudView.ShowAchievementBanner` — the unlock toast: gold border, trophy glyph, held longer than the transient banner. Takes the **topmost** toast slot rather than stacking below the bankruptcy and transient banners — those fire from upkeep at turn start, an unlock fires at game end, so they cannot co-occur.
