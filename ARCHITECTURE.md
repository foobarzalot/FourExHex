...
The tutorial popup is a bottom-anchored, autowrap, click-through
panel managed via `ShowTutorialMessage` / `HideTutorialMessage` (no
`Refresh`-driven state). It surfaces three sources of text during
Tutorial Preview:
...
- Per-beat step instructions ("Press the Buy Peasant button.",
  ...
  `TutorialPreviewCues` at the tail of every `Apply()`.
