...
- **Controller (Godot-free, unit-tested):**
  - `AchievementCatalog` — the enumerable definition table, and the single list a platform-console registration would derive from. `Target` derives the kind: 1 is boolean, above 1 a counter (mapping to Game Center percent and Play Games steps). Each row carries its own `Advance(AchievementEvent) → int` predicate, so adding an achievement is one table entry. Lives here rather than Model because the display keys are `StringKeys` constants.
  - `AchievementTracker` — applies one `AchievementEvent` to the store: skip if already unlocked, else `min(stored + delta, target)`, report, and unlock on reaching target. Returns the newly unlocked ids.
...
- **Scripts (Godot view layer, test-excluded):**
  ...
  - `AchievementsPanel` (`scripts/AchievementsPanel.cs`) — a `CanvasLayer` modal on the `CreditsPanel` skeleton, reached from the main menu's **Achievements** button. Rows rebuild on open. Earned rows ink gold; counters show `current / target`.
...
**Award call path.** `GameController` owns the funnel: `GameOperations.CheckGameEndConditions` → `HandleGameEnded` → `AwardEndOfGameAchievements` → `GameEnded?.Invoke()`, so subscribers (the campaign result recorder, the trailing `RefreshViews`) see the updated record in the same frame. The award raises `AchievementEvent.GameWonByHuman` only when `SessionState.Winner` resolves to a `PlayerKind.Human` seat — a stasis end and a `PlayerId.None` winner (Viking wipeout) are both "nobody won". Any human seat counts; the record belongs to the device.
