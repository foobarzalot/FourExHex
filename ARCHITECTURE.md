...
- **`src/FourExHex.ViewMath/FourExHex.ViewMath.csproj`** — plain `Microsoft.NET.Sdk`, **no GodotSharp**, one-way `<ProjectReference>` to `FourExHex.Model` (primitives like `HexCoord`). Godot-free view math needing floats: `DisplayScaleMath`, `SafeAreaMath`, `MapPlacement`, `PanMath` (inset-aware play-area center + pan clamp: each axis clamps into the range between the board's two viewport alignments, widened outward by the scroll pad — travel is |slack| + 2·pad, so a fitting axis pans across its slack instead of pinning centered), `CameraFocusMath` (comfort-zone test for the demo-replay camera follow), `FogBackdropMath` (half-extent of the Fog Of War backdrop square: board diagonal + max viewport dimension / zoom floor + scroll pad, so it covers every pan-clamp extreme at any rotation), `ZoomMath`, `ScreenLayout`, `HudPanelMath` (HUD-panel sizing: width clamped to viewport, height to fit wrapped text), `KeyboardAvoidance` (mobile keyboard panel lift), `MultiTouchTapDetector` (3-finger-tap debug cheat menu; fires on third concurrent touch, re-arms after all lift), `TapSlopDetector` (tap-vs-drag for a tappable control inside a scroller: a press that travels more than `SlopPx` is the scroller's to handle, not an activation — fed GLOBAL positions, since a scroll drags the content under the finger and leaves local coordinates nearly still; `Cancel` drops a gesture whose host rebuilt mid-press), `EditorPaletteLayout` (brush-grid column wrapping), `HudCornerLayout` (`CornerGap`/`CornersFit`: clear space left between the two oppositely-anchored HUD corner zones — negative means they overlap; `SideOffset` backs corner chrome out of a landscape notch by `CornerNudgeFactor` (a partial fraction of the side inset, deliberately less than the rails' full inset), and the inset-aware `CornerGap` overload budgets against those nudged positions; `RailSideOffset` places a landscape rail so its buttons land on the corner pad, level with the corner strips at zero insets, while taking a notch in full), `HudBottomStripLayout` (the landscape bottom stack, measured up from the viewport edge: `LabelBottomOffset` = the pad for the seed label in the corner, `StripBottomOffset` lifts the undo/redo and End Turn strips above the label by its measured height + `LabelGapPx`, `RailClearance` = label + gap + button + gap backs both expanded rails off the baseline; `CornerBottomOffset` is `safe.Top + pad`, mirroring the top chrome on purpose so the home-indicator band stays tap-through), `ThumbnailLayout` (contain-fit for offscreen board viewports — `FitInside` plus `OrientedFit`, which swaps the grid aspect tall in portrait so the hosted HexMapView rotates like the in-game map; shared by the New Game thumbnail and the Instructions demo view), `StepperMath` (integer −/value/+ stepper logic: snap-to-step clamp, neighbour offset, digit parse), `ListNavMath` (pick-one list keyboard navigation: `Step` moves a selection index with the ends as walls — index −1 enters at first/last — and `ScrollToReveal` returns the minimum scroll that brings a row fully into view), `KeyRepeater` (held-key auto-repeat schedule: one step at the initial delay then one per interval, accumulating elapsed time so short frames can't round down to nothing and a hitched frame still owes every step), `HudTourSteps` (wrap-around cursor over the ordered guided-UI-tour steps — `Next`/`Prev` wrap, `JumpTo(step)` for click-to-jump), `PanelFitMath` (centered-panel shrink-to-fit: `AvailableBox` / `ScaleToFit` (never upscale) / `WidthFitWithHeightCap` (Credits) / `CappedFill` (landscape chrome) / `ContentShrinkScale` (quantized play-config content scale with grow hysteresis) / `CardInteriorWidth` (reflow-to-safe-box card width, floored — the guided tour) — shared by MainMenuScene, SlotPickerDialog, SettingsPanel, CreditsPanel, LandscapeMenuChrome, HudTour), `HitTestMath` (`InOffsetBounds(col, row, cols, rows)`: is an offset coord on the Cols×Rows board — hover/paint gating + water-rim skip), `EasingMath` (`SmoothStep(t)` clamped-`[0,1]` ease-in/out + `Lerp(a, b, t)`, driving HexMapView's camera-pan animation), `HexRounding.Round(float, float)`, `HexProjection` (pointy-top pixel↔axial board geometry: `ToPixel`/`FromPixelFrac`/`FromPixel`, `FirstHexCenterOffset`, `HexExtent`, `GridPixelSize` — the single source the view draws, frames, and hit-tests through). Pressure-relief valve for the no-floats rule below.
...
- **`tools/FourExHex.LevelDesigner/FourExHex.LevelDesigner.csproj`** — plain `Microsoft.NET.Sdk` console exe referencing Model + Controller; the headless level-design CLI (see **Level-design harness** and `LEVEL_DESIGN.md`). Thin arg-parsing + file I/O only. Its directory carries a `.gdignore` so the Godot editor never scans it.
...
- **Pixel projection is view-side.** `HexProjection` in `FourExHex.ViewMath` owns the pointy-top board geometry — `ToPixel` / `FromPixelFrac` / `FromPixel`, plus `FirstHexCenterOffset`, `HexExtent`, and `GridPixelSize`. `HexRounding.Round(float qFrac, float rFrac) -> HexCoord` is the float→int boundary keeping `HexCoord` integer-only in Model. Three callers share `HexProjection` and must agree exactly, since a board framed by different constants than it is drawn with drifts off-center: `scripts/HexPixel.cs` (a thin `Vector2` shim over it), `MapPlacement.ContentPixelBounds` (content-aware centering / pan-clamping), and `HexMapView`'s `PixelSize` + `FirstHexCenterOffset`.
- **`Log` is Godot-free** — routes through injectable `Log.Sink`, wired by `Main` to `GD.Print`. See **Logging**.
- **Save format is v21.** Ownership is a player index on the wire (−1 = `None`); claim-victory tiers persist by index. Gold/mountain are mutually exclusive on the wire (two bools); a tile with both normalizes to mountain-only on load; in-memory it's one `HexTile.Feature` enum that can't hold both (see **Mountain tiles**). Saves v2–v20 still load, absent fields defaulting (`IsGold`/`IsMountain` → `false`, `Difficulty` → `Soldier`, Rising Tides `Mode`/`PendingTide` → off/empty, Fog Of War `Seen` → empty, Viking Raiders `VikingAtSea`/`VikingNextWave`/`VikingLastRound`/`VikingLastSpawnRound` → empty/0, barbarian `IsAggro` on a unit's `OccupantDto` → `false`, achievement `RunStats` → all-zero), with legacy color-hex claim data and pre-v6 unit-level names migrated via `GameSettings` palette matching and `SaveSerializer.ParseUnitLevel`. A save carrying a `Replay` block also carries `ReplayVersion` (`SaveSerializer.CurrentReplayVersion`); on load a mismatched/absent stamp drops the replay (game state loads normally — the board is baked into the save), except a `Tutorial` save, whose replay is its content: that fails loudly.
...
CONTROLLER (pure C#) ─ GameController
  ...
  subscribes in ctor:
    ...
    hud.PreviousUnitClicked      → OnPreviousUnitPressed (Shift+N: same tier cycle backward, strongest first)
  ...
  click policy state machine:
    OnTileClicked → pending-mode branch (buy/build/move) or SetSelection branch. The buy and move branches
      ...
      Rejected clicks: in-range near-miss flashes + stays in mode; out-of-range flashes + cancels with
      Escape semantics (selection kept) — unless the click landed on another own territory, which falls
      through to selection (quick redirect). "In range" for buy/move = own territory or border-adjacent;
      for tower = own territory only.
  ...
  undo/redo:
    Each human handler wrapped in TrackHandler — pushes UndoEntry (game + session snapshot + run-stats copy) iff state changed.
  ...
  turn rotation:
    OnEndTurnPressed → undo.Clear → EndOfTurnProcessing (win check only)
      → AdvanceToNextActivePlayer (skip capital-less; the trailing
        neutral seat — TurnState.IsNeutralSeat — is exempt)
...
MODEL / STATE (pure C#)
  ...
  SessionState ─
    ...
    Undo (UndoStack of UndoEntry = GameStateSnapshot + SessionStateSnapshot + RunStats copy)
...
VIEWS (Godot Nodes)
  ...
  HudView : CanvasLayer, IHudView
    ...
    Guided UI tour (from the Help menu):
      ...
      buy row collapses to one step); order cursor is HudTourSteps (ViewMath). The
      card fits the safe box on viewport resize and SafeArea.Changed (FitCards):
      its interior width is PanelFitMath.CardInteriorWidth(440, availW, stylebox
      chrome) so the body label reflows instead of overflowing the carousel's
      ClipContents, with a deferred PanelFitMath.ScaleToFit uniform shrink as the
      backstop. Logs "[tour] enter/step/exit/fit" (Hud).
...
MODEL PRIMITIVES
  ...
  TurnState — Players[], CurrentPlayerIndex, TurnNumber; SeatCount =
              Players.Count + 1 — every round ends on the neutral seat
              (IsNeutralSeat, CurrentPlayer = Player.Neutral), and the
              wrap past it increments TurnNumber
  ...
  UndoEntry — (GameStateSnapshot, SessionStateSnapshot, RunStats copy)
...
## Procedural trees, mountains, gold, neutral region & territory clumping

`MapGenerator.BuildInitialGrid` scatters trees, mountains, and gold onto a fresh map, each driven by an integer **density** (percent of land) on `MapGenOptions`, plus **`ClumpingFactor`** (0..100) shaping how player ownership is assigned and **`NeutralDensity`** (0..75) carving a neutral region (`MapGenOptions(TreeDensity = 5, MountainDensity = 0, GoldDensity = 0, ClumpingFactor = 0, NeutralDensity = 0)`). The record threads through `BuildInitialGrid(..., MapGenOptions? options = null)` and `ProceduralGame.Build(..., options)`. With `MapGenOptions.None` (and the no-options overload for tests/replay): density 0 gates the mountain/gold/neutral passes (zero RNG draws), `ClumpingFactor 0` gates per-cell-random owner assignment, and the **tree default 5%** scatters `land.Count * 5 / 100`. All densities are percent of `land.Count`; all scatter/clumping math is integer (no floats) and deterministic in the seed.
...
- **Neutral region** — `NeutralDensity` (0..`MapGenOptions.NeutralDensityMax` = 75) is the **total neutral coverage** target: the percent of land left unclaimed, features included. Gated on `NeutralDensity > 0`; when on, `BuildWithNeutralRegion` **inverts** generation: every land tile starts `PlayerId.None` → mountain/gold scatter (unchanged) → each player gets an **equal cell quota** (`(land − land×density/100) / playerCount`, ±1 — exact side balance by construction) → **seeds** spread farthest-point (count interpolates with `ClumpingFactor` exactly like `AssignClumpedOwners`: 100 → one seed per player, lower → toward per-cell noise; dealt round-robin; gold-averse) → players **expand in strict rotation**, one adjacent unclaimed cell per turn (rng pick from a per-player frontier) until every quota is met → the **unclaimed remainder is the neutral territory**. **All land is claimable**: mountains freely (the landmass is one connected component, so terrain can never wall a player in — generated maps can start with player-owned mountains), gold **only as a last resort** (it is the contested prize; expansion and seeds steer around it, so gold overwhelmingly stays neutral). A player walled in by neighbors restarts **once** in the largest unclaimed region; a second walling — or a lone final cell — **forfeits** the remainder instead (the cells stay neutral). Hard guarantees: **at most two coherent regions per player, never a stranded 1-tile island**; under extreme contention (feature knobs near their caps with a tiny neutral knob) forfeits can leave total neutral above the knob. **Viability backstop**: every player must own two adjacent tiles (a capital-capable start); an rng-free repair flips plain-neutral cells to any starved player (`Warn` when even that is impossible). Every rng-coupled pick draws from a sorted list. Instrumented `[mapgen] neutral: density=… neutralTarget=… features=… playerCells=… seeds=… reseeds=… forfeited=… neutral=…` Debug line (category `MapGen`).
...
- **Campaign terrain is per-level, not the freeform steppers.** `CampaignProgress.MapGenOptionsForLevel(level)` derives a level's densities deterministically from the level number: mountains present ≈55% (density 10 else 0), gold present ≈45% (density 5 else 0), trees vary across {0, 5, 10}%, and **clumping drawn from the shared `ClumpingFactorStops`** — so terrain and feel are fixed and reproducible (same level → same options → same seed → same map). The clumping draw is sequenced last so it doesn't perturb tree/mountain/gold values; Viking Raiders levels then clamp it to ≥90 (`VikingMinClumpingFactor` — fragmented starts are near-unwinnable against raiders) and Fog of War levels lock it to 100 (`FogClumpingFactor` — fog plays best as one compact blob per player exploring unclaimed ground). **Unclaimed land couples to clumping** (`NeutralForClumping`): Freeform and Fog of War levels derive `NeutralDensity` as a pure function of the final clumping stop — 90 → 50%, 95 → 60%, 100 → 75%, below 90 → 0 — with no rng draw of its own; Rising Tides and Viking Raiders levels always get 0. Every Fog of War level is therefore a one-blob-per-player, 75%-unclaimed map. `Main` uses it whenever `GameSettings.CampaignLevel` is set (freeform falls back to the steppers); the campaign confirm-sheet preview renders the same derivation via `MapThumbnailView.RequestRandom(seed, options)`.
...
- A **shore** tile has **<6 in-grid neighbours** (`ShoreTilesOf`). Selection always takes the most exposed tiles first — highest `WaterBorderWeight(grid, coord) = 6 − in-grid neighbours`; a more-exposed tile is never passed over. Only the **equal-exposure tie-break** varies: ascending `HexCoord` when `rng` is null, else a seed-deterministic shuffle within each exposure band (see **Randomized selection**). `GameOperations.ForecastTideForCurrentPlayer` / `MaybeRiseTidesFor` always pass `_rng`. A **mountain** shore *demotes* (`IsMountain=false`) without sinking; a non-mountain shore is removed + watered.
- **Budget = tide level**, escalating: `SubmergeBudgetForRound(round)` = `1 + (round − 1) / GameSettings.TideRiseIntervalRounds` (interval default 6, integer-only) — rounds 1–6 sink 1 tile per player-turn, 7–12 sink 2, and so on; `RoundsUntilTideRise(round)` is the countdown to the next increment. Derived purely from `Turns.TurnNumber` — no stored counter, so saves and replays need no extra field. Both `ForecastTideForCurrentPlayer` and `MaybeRiseTidesFor` pass it, so real and phantom turns escalate identically. `FOUREXHEX_TIDE_INTERVAL` overrides the interval for headless sweeps (`Main`, `TowerKillValues`-style; the model never reads the environment). Each forecast logs `[tide] T{round} level= risesIn=` under `Tide`.
...
**Turn-start focus (human).** At every human turn start with a pending forecast, `GameController.TryFocusPendingTide` (branched to from `SelectTerritoryForHumanTurnStart`, winning over the restore-or-fallback selection) selects the territory containing the doomed tile and centers the camera on the tile via `IHexMapView.CenterOnCoord`, so the per-turn erosion is never off-screen. With a multi-tile forecast the focus tile is the **lexicographically smallest** doomed coord (`PendingTide[0]` is the exposure-ordered pick, not a stable choice). This is the only camera movement any human turn start makes. The territory is selected even when it has nothing actionable; a capital-less singleton pans without selecting; mountain-demote steps focus the same way. Logs `[tide] turn-start focus` under `Tide`. The same turn-start seam then shows the tide banner (`TideBannerContent.For`, unit-tested copy: "Tide level N — rising in K turns", singular/plural by K) via `IHudView.ShowTransientBanner` — the Viking wave banner's slot and guards; the modes are mutually exclusive, so the banners never compete.
...
A visibility restriction (`GameMode.FogOfWar` on `GameState.Mode`): AI behaviour and determinism are identical to Freeform — the AI never reads `Seen`, and the only rules that branch on fog are the win gates below. Requires **exactly one human**; freeform Configure Game locks slot 0 to Human and restricts the other slots to Computer/None (`MainMenuScene.ApplyGameModeRoleLock` — 1–5 computer opponents), and the single-human campaign roster satisfies it for free.
...
**Claim gating.** You can't declare the board yours over ground you've never laid eyes on: while `GameOperations.HiddenLandActive`, `OnEndTurnPressed` withholds the claim-victory prompt. **Only that prompt is gated** — the two awarded wins are untouched, because neither can outrun the human's knowledge: a domination sweep means owning every tile, hidden ones included, and an AI can only take the board once the human is eliminated, which reveals the map; the end-of-turn win needs every opponent's capital destroyed. `VisibilityRules.HiddenLandCount(state)` counts land tiles at tier `Fog` that the human does not own — owning a tile is knowing it (sight comes only from capital-bearing territories, so an isolated singleton is owned yet unseen), and water is excluded structurally since water coords are not in the grid, so an unexplored ocean never withholds the prompt. It is pure: unlike `UpdateSeen` it marks nothing, so asking never grants sight. The count is 0 whenever `SoleHumanPerspective` is null (not Fog Of War, not exactly one human, or the human is eliminated), so all-AI runs — the `FOUREXHEX_6AI` diagnostic modes, the campaign winner sweep — keep ordinary rules and byte-identical output. Because sight reaches one hex beyond owned territory and no further, a revealed map means the human's territory has bordered every land tile, which puts the prompt late: the lower tiers rarely fire in fog. A one-shot `[fog] map fully revealed` Info line (category `Fog`) marks the turn the prompt becomes available; it prints only on a real transition, so a game that never had a fog perspective stays silent.

**Rendering (`HexMapView`, all guarded so non-fog modes are unchanged).** Fills, outlines, and territory borders paint from `EffectiveOwner` — live owner when the tile is in current sight, else neutral `None` (stale terrain reads grey; fog tiles are hidden anyway). The occupant pass skips non-visible tiles (no owners/occupants leak). One baked `TriangleSoup` `_fogLayer` (topmost) covers the full water extent: opaque cool **mist** over never-seen cells, a translucent cool **memory wash** over stale cells; static terrain/water/decorations show (dimmed) under stale and are hidden under fog. Tier frontiers are **feathered** via per-vertex alpha, drawn as a fan from a full-tint centre vertex (`TriangleSoupBuilder.AddFan`) so the gradient stays spike-free. `ShowFog` diffs the visible set and only repaints (`RepaintFogVisuals`) when it changes. Beyond the rim the viewport clear colour is the sea colour, so while fog is active `HexMapView._Draw` paints an opaque `FogCoverColor` square beneath every child layer (water bake included), sized by `FogBackdropMath.HalfExtent` to the farthest the pan clamp can carry the viewport at the zoom floor; `UpdateFogBackdrop` re-queues it only on a fog toggle or a zoom-floor change (`[fog] backdrop …`, category `Fog`).
...
**State.** `GameState.Vikings` (`VikingState`, always non-null, default-empty outside the mode): `AtSea` (lex-sorted `SeaViking(Coord, Level)` list — water coords have no `HexTile`, so raiders at sea live off-grid), `NextWaveIndex`, `LastCompletedRound`, `LastSpawnRound`. Excluded from undo snapshots (it mutates only during the neutral seat's turn and the undo stack clears at end of turn; landed vikings are ordinary grid occupants and snapshot normally); the replay rewind resets it via `VikingState.Reset`.

**Schedule & rules (`VikingRaidersRules`, Model, integer-only).** Waves spawn at the neutral seats of rounds 2, 5, …, 17 (`FirstWaveRound`/`WaveIntervalRounds`/`TotalWaves`; `WaveDue` catches up a missed spawn next round) — each wave is visibly offshore for the whole of the following round and lands at that round's neutral seat; composition is a fixed table independent of map size (`WaveComposition`, totals 10 → 15): 5 Recruits every wave (`WaveRecruits`), Soldiers 5/6/7/8 then frozen (`WaveBaseSoldiers`), Captains 1/2 from wave 5 (`FirstCaptainWaveIndex`), never Commander, strongest listed first so they claim the best landing spots; spawns clamp to available coastal water. `CoastalWaterCoords` (water coords with a grid neighbour) hosts `ChooseSpawns` — a "landing party" placement, sequential per raider with a lexicographic preference: any viable landing at all for the raider's level first (a zero-target spot is certain death), then a landing zone that does NOT overlap the raiders already placed this wave (overlap is a last resort among landable spots), then the most viable landing neighbours, then shortest distance to the party, remaining ties drawn from the seeded rng; one raider per coord. `DisembarkTargets`: player-owned neighbours under the ordinary capture threshold (`DefenseRules.Defense < level`, radiation included), plus already-neutral neighbours with no Unit/Tower (trees/graves land like a reposition — no defense check against the vikings' own side). `ThreatRemains` = raiders at sea ∨ waves pending ∨ landed None-owned units (`LandedRaidersExist`); always false outside the mode, since it is the win-suppression predicate and a stray raider must never make another mode's game unwinnable. Raider actions themselves are **not** mode-gated: any landed raider acts on the neutral seat in every mode, from round 1's seat onward — what it *does* there depends on its aggro state (below). A wave raider disembarks aggro; a raider seeded by the map editor's unit brush or the `MapGenerator` barbarian scatter starts passive.

**Barbarian aggro (`Unit.IsAggro` + `BarbarianRules`, Model).** Every None-owned unit is a *barbarian* with a one-way per-unit aggro flag, false by default. A **passive** barbarian never expands: on the neutral seat it takes at most one random **wander** beat per turn — a plain `AiMoveAction` reposition drawn uniformly (via the seat's per-turn rng, at choose time) from the empty tiles of its own neutral territory *plus one "hold" slot* — executed and replayed through the ordinary viking-move path (`ExecuteVikingMove` / `ReplayVikingMoveBeat`, destination baked in so playback consumes no RNG). The wander loop guard is `AiTurnDriver._vikingWandered` (reset with `_aiVisited`; repositions never set `HasMovedThisTurn`, so without it a wander would re-enumerate to the step cap). An **aggro** barbarian runs the expansion path below. Flips are state-derived (never made inside a chooser, so replays re-derive them) and propagate through `BarbarianRules.PropagateAggro`, called after every territory recompute by both `GameOperations.HandleCapture` and `AiSimulator.Reconcile` — 1-ply lookahead predicts the flip live play performs (pinned by the sim↔live parity tests). Three triggers: **compromise** — a capture takes any coord of a previously-neutral territory, flipping its surviving units (a tile lost to the tide or turned neutral is not a compromise); **contact** — any neutral territory containing an aggro unit flips wholly (covers an aggro raider's capture merging its tile into a passive territory, and a raider's neutral-side landing); **cornered** (Rising Tides, neutral seat's turn start, after the forecast locks) — `BarbarianRules.AggroCorneredByTide` flips a territory whose doomed passive unit has no non-doomed in-territory escape; until then a doomed unit always retreats (its wander draw excludes doomed tiles and the hold slot). Computer players avoid provoking: `AiStateScorer.BarbarianProvokePenalty` (2× the summed value of the units that would wake, logged `[barb-avoid]`) is subtracted from move/buy candidate deltas in `ComputerAi.BestDeltaAboveThreshold` — ranking-only, so a lone provoking option is still taken. The flag rides undo snapshots and `AiSimulator.Clone` via `HexOccupant.Clone`, serializes as an omitted-at-default `OccupantDto` field, and appends `:aggro` to a unit's checksum row only when set. Headless levers: `FOUREXHEX_BARBARIAN_DENSITY` seeds passive Recruits on neutral land (see the mapgen overrides in `CLAUDE.md`) and `maps/barbarian-pass.json` is an authored chokepoint scenario; `[barb]` wander/hold/aggro lines log under category `Viking` (census counts on the seat's turn-begin line).

**The neutral seat's raider turn.** All raider action runs inside the neutral seat — the round's **final** rotation seat (`TurnState.IsNeutralSeat`, `Player.Neutral`), after the last colored player and before the wrap that increments `TurnNumber`. It is an ordinary AI turn: the three Advance→Start seams are plain `AdvanceToNextActivePlayer(); StartPlayerTurn();`, and `AiTurnDriver` drives the seat through the same paced preview/execute machinery (and the chunked Instant track), swapping in `VikingAi.ChooseNext` as the chooser whenever `IsNeutralSeat` — with **no acting-territory highlight for any viking beat**, live or replay (`ResolveAiActingTerritory` and `ResolveReplayActingTerritory` resolve null on the seat: sea beats have no territory and landed beats would blink the neutral land; the shield visuals and travel tweens carry the action) — in strict order: disembark earlier-round raiders (best landing by 1-ply clone+score, coming ashore aggro; `VikingPerishAtSeaAction` when every landing is blocked) → passive-barbarian wander beats (`VikingAi.ChooseWander`, one per unit, holds included — see **Barbarian aggro**) → aggro landed moves via `ComputerAi.ChooseNextAction(state, PlayerId.None, …)` with passive territories masked into the visited set (capital gate bypassed for None; captures only — own-territory trees are never chopped, and the economy (2a–4a) and defensive-reposition (4b) phases are skipped, so an aggro raider with no capture holds; `MovementRules` bans None combines; `UpkeepRules.TotalUpkeepFor` returns 0 for None so vikings never bankrupt into graves; `AiStateScorer` skips the own-tree penalty and the bankruptcy zeroing for None, so viking units read as real threats in every player's scoring) → spawn a due wave **last** (`VikingState.LastSpawnRound` keeps a fresh wave from acting on its spawn round — exactly one round of warning; a wave scheduled for round N spawns at round N's seat and lands at round N+1's). Only Viking Raiders runs the sea half — `VikingAi.ChooseNext` gates the disembark and wave-spawn beats on a `seaborne` flag; every other mode reaches the sequencer solely for landed raiders, which move through the same middle beat. The seat's turn start is the standard `StartPlayerTurn`: `ReseedRngForCurrentTurn` uses the seat's own index (`Players.Count`), `ResetMovementFor` un-spends the landed raiders, and sea graves from the prior round wash away; its `EndOfTurnProcessing` stamps `VikingState.LastCompletedRound` (this mode only — a stamp in another mode would leak the viking block into that game's save and canonical string) and, if no capital survives, declares `PlayerId.None` the winner ("The Vikings have conquered the island!") before the advance seam runs. A viking capture that eliminates a human raises the ordinary defeat overlay and pauses the turn; Continue re-arms it (`OnDefeatContinuePressed` schedules the driver whenever the current player is AI). `GameOperations.HumanInputLocked` (= `InSilentAiBatch() || Turns.IsNeutralSeat`) gates every mutating human handler for the whole seat; overlay-dismiss handlers stay live.
...
**Persistence & replay.** The save carries `VikingAtSea`/`VikingNextWave`/`VikingLastRound`/`VikingLastSpawnRound` (all omitted at defaults, so non-viking saves' wire format is unchanged) and each landed unit's `IsAggro` on its `OccupantDto` (omitted when passive). Replay records the neutral seat's beats as ordinary beats with `Actor = Players.Count`: viking actions as `VikingMove`/`VikingDisembark`/`VikingPerish`/`VikingSpawn` (explicit placements; playback consumes no RNG) and the seat's end-of-turn as a plain `EndTurn` beat. A `ReplayVersion` stamp that doesn't match `SaveSerializer.CurrentReplayVersion` drops a save's replay block and fails a tutorial loudly. The `VikingTurnEnd` beat kind exists only so version-1 files materialize before the gate rejects them — playback ignores it. `BeginReplay` rewinds `VikingState` to defaults. `GameStateChecksum` appends a viking block only when non-default. Pinned by the VikingRaiders `ReplayFidelityTests` rows and the shipped-recording drains in `TutorialReplayFidelityTests`.

**Rendering & shell.** Viking units carry a "painted shield" glyph (`HexMapView.CreateVikingShieldVisual`): a top-down cream shield with an ink rim and center boss, rank shown by a doubling ladder of painted segments — half-painted (Recruit), quartered (Soldier), eight segments (Captain) — in a fixed ink `#111111` / cream `#EFE6D4` palette that is never player- or actionability-colored. The rank segments are the aggro tell: ink on a passive barbarian, bright battle red (`VikingAggroRed` `#D92B2B`) on an aggro one; rim, boss, and base are identical in both states, and raiders at sea always paint the red variant (a sea raider exists to land and raid). Identical on land and at sea; raiders at sea render in `_seaVikingsLayer` via `IHexMapView.ShowSeaVikings` (pushed each `RefreshViews`) directly on the water. A perish leaves a grave marker on the water (`VikingState.SeaGraves` — transient: not saved, not checksummed, not snapshotted; replay re-derives it from perish beats) drawn with the land bankruptcy choreography (shield shrinks out on the deaths layer, grave grows in staggered beneath) and the Rising Tides `TileSubmerged` "bloop"; the graves wash away when the next viking turn begins (or at phase end if the threat just cleared — a grave on water never becomes a tree). A wave's spawn plays the "ripple rise" entrance (`VIKING_SPAWN_ANIMATION.md`): each shield scales in from nothing with a Back-ease overshoot while fading in, two flat white rings ripple outward beneath it (`SeaSpawnRippleRings`, constant-stroke `_Draw` arcs; rings gated on `UserSettings.VfxEnabled`), staggered 0.15s per raider, suppressed under silent mode and for one pass after a full rebuild (`_animateSeaSpawns`); the arrival is sounded once per wave with the ElevenLabs-generated creaking-longship cue (`SoundEffect.VikingArrival` → `assets/audio/viking_arrival.wav`, manifest entry in `tools/generate_sounds_eleven.py`). The turn-order swatch bar renders the neutral seat as one extra grey swatch whenever it can visibly act — Viking Raiders mode, or landed raiders on the board (`VikingRaidersRules.LandedRaidersExist`): the highlight simply follows `CurrentPlayerIndex` (the seat index equals `roster.Count`), and in Viking Raiders the swatch dims like an eliminated player once the threat clears. In other states the seat exists but draws no swatch; the highlight index clamps to the rendered bar. At every human turn start the controller shows a transient wave banner (`VikingWaveBannerContent.For`, unit-tested copy: "Wave X/Y arriving in N turns" countdown, where N counts to the turn the raiders become visible offshore — one past the spawn's due round / "Wave X/Y" while the wave sits offshore / "Final wave" variants) via `IHudView.ShowTransientBanner` — a click-through top-center toast that fades in, holds, and fades out on its own; schedule-resistant because the countdown derives from `VikingRaidersRules.RoundsUntilWaveDue`, itself defined purely through `WaveDue`, and turn-1 sequencing after the one-time mode intro falls out of `Main` deferring game start until the intro is dismissed. First-encounter explainer via `GameModeIntro`; selectable from the **Game Mode** dropdown; `FOUREXHEX_MODE=VikingRaiders` forces it for headless runs.

## Randomized selection

Every game breaks selection ties with seed-deterministic random picks (integer-only RNG) at three selection points: the **in-tier capital choice** (`CapitalPlacer.Choose`) and its equal-old-size merge tiebreak (`CapitalReconciler.Reconcile`), the **equal-exposure Rising Tides submerge tie-break** (`RisingTidesRules.ForecastSubmerge`), and the **AI equal-delta candidate tie-break** (`ComputerAi.BestDeltaAboveThreshold` — a uniform pick among the candidates tying the phase's best score delta, so different master seeds play differently on identical boards). Priority order always holds — capitals respect the occupant tier, tides take the most-exposed tile, a strictly better-scored AI candidate always wins; only the choice *among equals* randomizes. The lex-min null-rng path in `CapitalPlacer`/`CapitalReconciler` remains for editor paints and test fixtures; every in-game reconcile passes `randomize: true`.

- **Two RNG sources, by constraint.** Capital placement seeds a local `DeterministicRng` from the territory's own coords (`CapitalReconciler.SeedFromCoords`, FNV-1a + splitmix32 avalanche), so it consumes nothing from the live per-turn `_rng` stream and the AI's cloned 1-ply simulation reproduces the identical replacement capital (`AiSimulatorDriftTests` pins sim == real). The tide tie-break draws from `GameOperations._rng` at forecast time — the first per-turn RNG consumer, right after `ReseedRngForCurrentTurn`, so it lands at a fixed stream offset and reproduces on resume/replay; the simulator never touches tides. The AI tie-break draws from the same per-turn `_rng` after that, at most one `NextBounded` per decision and only when a real tie exists (a sole winner consumes nothing), so every draw folded into `StreamHash` maps 1:1 to an actual tie-break (`[tie-break]`/`[chose] ties=N`, category `Ai`, Debug). Still seed-deterministic since the board itself evolves from the master seed.
- **Replay compatibility is version-gated.** A recording's capitals/tides are re-derived during playback, so a replay is only faithful under the tie-break rules that recorded it — `ReplayVersion` (see *Save / load*) drops replays stamped by a different generation instead of replaying them divergently. The `(mode)` matrix in `ReplayFidelityTests` pins record → serialize → replay checksum equality; `TutorialReplayFidelityTests` drains every shipped `tutorials/*.json` demo to completion. The drain covers playback only — playback consumes every recorded beat as an end-turn regardless of actor, so it can't see a Preview-side cursor stall. `TutorialPreviewPlaythroughTests` closes that gap: it plays the shipped tutorial through the Preview wiring as real UI input (clicks, Buy / Build Tower / End Turn, narration taps) over a `QueuedAiPacer` — the deferred cadence the real pacer uses — and fails with the parked beat's identity if the script runs out of actionable work.
...
- **`ReplayBeatPreviewing` event** — raised by the paced track as each beat's preview begins (the instant track raises nothing — it does no preview or camera work), forwarded as `GameController.ReplayBeatPreviewing`; the demo-replay camera follow pans on it.
- **Turn-end fast-forward** (`fastForwardIdleTurns` ctor flag, set only by `DemoReplayScene` and `InstructionDemoView`): the paced track executes turn-end beats with no preview (no highlight, no `ReplayBeatPreviewing`, zero delay) and `ScheduleNextReplayBeat` passes `StepPacing.ReplayIdleTurnSkipMs` as the boundary redispatch delay, so idle opponent turns flick past and contentful turns run nearly back-to-back. Main's replays and Tutorial Preview keep the full authored cadence. Pinned by the `ReplayIdleSkip` rows in `GameControllerTests`.
...
**Move-arrival pipeline** (view-side, keyed off `AnimateUnitMove`): on a paced move the entire destination-side application holds while the source glyph tweens to the destination — the capture rebuild (tile recolor/borders), highlight, occupant refresh, fog/tide/sea pushes, and destination-anchored FX/sounds stage themselves, then apply together when the tween lands (`CompleteArrival`), so impact happens at arrival instead of at mutation time. The application runs in inline-`RefreshViews` order: staged rebuild + highlight, then staged fog (`ApplyStagedFog` — before the occupant refresh so occupants paint under the new projection), then the occupant refresh, then tide/sea and one-shot FX. A rebuild applying from the staged path preserves the move-target layer (`preserveTargets`) — previews alive at arrival were painted after the hold began (auto-advance, buy streak) and are fresher than the rebuild; inline rebuilds keep the clear because the handler repaints targets afterwards. Travel duration is `StepPacing.MoveTravelBaseMs(distance)` × the shared speed-multiplier probe (`StepDelayMultiplierPercent`; 0 = Instant = snap), easing Sine/InOut. Snap paths (silent batch, Instant, fog-hidden source, missing glyph) apply inline — a snap IS the arrival. Async calls mid-flight (undo, input) land the travel instantly first; same-frame follow-up calls (e.g. `TrackHandler`'s trailing refresh) fold into the hold. Verified via `Anim:Debug` `[move-anim] hold/travel/arrival`, `Input:Debug` `[targets]`, and `Fog:Debug` `[fog] staged/apply` lines.
...
**Invalid-tap policy.** A tap on an invalid target with a buy/build-tower/move action pending flashes rejection, then splits on reachability: an in-range near-miss (own-territory occupant, adjacent over-defended enemy; for tower, any invalid in-territory tile) stays in mode so the user can adjust without re-pressing the button; an out-of-range tap calls `CancelPendingAction()` (clears `Mode`/`MoveSource` + preview overlays, like Escape) and keeps the selection — unless it landed on another territory the player owns, where it falls through to the normal-selection block (quick redirect). Off-grid taps mid-placement flash + cancel + keep the selection; with no action pending they deselect. `OnTileClickedBody` routes the decision through `TryHandlePendingUnitAction` (buy + move) / `TryHandlePendingTowerAction`, both ending in the shared `CancelModeAndConsumeUnlessOwnRedirect`; `OnOffGridClickedBody` has its own branches (no validity or reachability step — an off-grid coord is never a legal target).
...
event Action? NextUnitClicked;         // N / next-unit button: cycle power tiers
                                       ...
                                       // tier), then one "off" step, then repeat.
                                       // Each tier pick turns on
                                       // SessionState.RepeatedMovement.
event Action? PreviousUnitClicked;     // Shift+N — same cycle backward
...
- **ViewMath — `UiMetrics`** (cross-cutting feel tokens: touch-button size,
  gutter, viewport margin, long-press threshold, CTA pulse cadence, plus the
  HUD corner-zone geometry the layout and its overlap guard must agree on —
  zone separation/edge pad, chip padding, current-swatch size, and the turn /
  gold label width floors) plus
...
- **Map badge (`HexMapView.RedrawWarningBadges`)** — a top-most `WarningBadgesLayer` stamps triangles on the capital of every affected current-player territory: red for `BankruptNextTurn`, yellow for `NegativeDelta`. Runs every `RefreshOccupantVisuals`; clears and returns if `HexMapView.EditorMode`, `RecordingMode.Active`, or `state.Turns.CurrentPlayer.IsAi`, else iterates `state.Territories`. Selection-independent. `EditorMode` is set by `MapEditorPanel` from its `PaintingEnabled` bit: an authoring draft has no turns and an empty treasury, so every painted garrison would otherwise classify as `BankruptNextTurn` and stamp a permanent triangle on its capital.
...
`AdvanceToNextActivePlayer()` calls `TurnState.EndTurn()` (the rotation runs every colored player then the neutral seat; the wrap past the seat increments `TurnNumber`) then runs `SkipEliminatedCurrentPlayers()`: loop while the seat is not the neutral one and `WinConditionRules.IsEliminated(currentPlayer.Id, grid)`. The eliminated player takes no input or AI action but isn't silently skipped: each iteration runs a "phantom turn" ticking tile-bound rules — `TreeRules.RunStartOfTurnGrowth` then `UpkeepRules.ApplyUpkeepFor` (orphan units bankrupt into graves). Income, view refresh, AI dispatch, and turn logging are skipped. Without this, an eliminated player's lone unit on a singleton would linger forever. The neutral seat is exempt from the eliminated check (it never owns a capital) and takes a real turn instead — including any viking-raider actions (see *Viking Raiders*).
...
|---|---|---|
| Recruit   | 9 / 18 / 27 / 36  | 14 |
| Soldier   | 10 / 20 / 30 / 40 | 15 |
| Captain   | 11 / 22 / 33 / 44 | 16 |
| Commander | 12 / 24 / 36 / 48 | 17 |

The bases step by exactly one gold per difficulty; the ×tier ladder amplifies
the step for higher units.
...
- **Shared player-setup screen.** Same page (`_playConfigPurpose` = `NewGame` | `EditorNewMap` | `TutorialBuilder`) feeds the procedural map page ("Next"), a new editor session ("Create Map" → `LaunchEditorNewMap`, handing kinds/difficulties via `MapEditorRequest`), or a builder session ("Create Tutorial" → `LaunchTutorialBuilderNew`, handing kinds + `GameSettings.Mode` via `TutorialBuilderRequest`). Only the forward action differs. The `TutorialBuilder` purpose is entered from the cheat menu (`TutorialBuilderRequest.ConfigRequested`, checked in `_Ready` before the landing page) and is exempt from the Fog Of War 1-human role lock — builder kinds only pick which colors are in play (`ShowPlayConfig` re-runs `ApplyGameModeRoleLock` so the exemption tracks the active purpose).

- **Load Starting Map / Load Map.** Both use the same picker (`SlotPickerDialog`, `previewMaps: true`) and launch straight into play (`LoadRequest.Pending` → game scene, baked roster) or editor (`MapEditorRequest.Pending = LoadMap`). No confirm. The play flow lists `SaveStore.ListStartingMaps()` — user maps plus the bundled catalog set (rows tagged "built-in" via `SaveSlotInfo.IsBundled`; a same-named user map shadows the bundled one) — and loads through `LoadStartingMap`'s user → `res://maps/` → `res://tutorials/` fallback. The editor flow lists `ListMaps()` (user only), so bundled maps are read-only.

- **Fill-to-cap surface (both orientations).** Portrait/landscape panels are a centered `LandscapeMenuChrome` surface filling the safe area up to a cap, sized by the single `ApplyPlayConfigLayout` path (shared by `FitPanels` and the `SafeArea.Changed` hook). Landscape caps `920×520`; portrait the transpose `520×920`. Container-based (`VBox`/`HBox`); portrait players use a two-line block, lists carry no `ScrollContainer` — content **shrinks to fit** instead (below).

- **Shrink-to-fit content scale.** Every fixed metric in the play-config page builders routes through `Sc` (heights/widths/separations, floored) or `ScFont` (fonts, 12-pt floor, applied to an `OptionButton`'s face *and* its `GetPopup()`) multiplying by `_configContentScale`. After each `ApplyPlayConfigLayout`, a deferred `CheckPlayConfigContentFit` measures both pages' `GetCombinedMinimumSize().Y` against the surface interior (`ApplyLayout`'s returned height minus 2×`LandscapeMenuChrome.ContentPadding`) and feeds `PanelFitMath.ContentShrinkScale` (ViewMath, unit-tested): shrink-only, floor-quantized to 0.05 steps, clamped ≥ 0.65, with 2 % grow hysteresis so floored re-measurements can't oscillate at the fit boundary. A changed target rebuilds the panel in place via `RebuildPlayConfigPanel` (shared with the orientation-flip path, which resets the scale to 1 first); a 3-rebuild chain guard `Warn`s and holds. Keeps the bottom nav (Back / Next / Start Game / Create Map) on screen on the smallest supported safe area (iPhone 13 mini). Logs `play-config fit` under `Render:Debug`.

- **Live thumbnail = offscreen `HexMapView` snapshot.** `scripts/MapThumbnailView.cs` renders the real `HexMapView` into a hidden `SubViewport`, snapshots to a static `ImageTexture` in a `TextureRect` — pixel-identical to Start Game, rendering only on change. `RequestRandom(seed)` builds via shared `ProceduralGame.Build`; `RequestMap(name)` loads a starting map via `SaveStore.LoadStartingMap(name).State` (user-then-bundled resolution); `RequestSlot(name)` an in-progress save via `SaveStore.LoadSlot(name).State`. Each request sizes the offscreen board to the state via `MapBounds.Infer` (authored maps may be any size; `Main` does the same before `HexMapView.Init`). Requests coalesced by a token so rapid requests snapshot only the latest. Refreshed on re-roll / map selection; under `Display:Debug`.
...
- **Load Game / Load Map preview.** `SlotPickerDialog` (shared by main-menu / in-game Load Game, Load Starting Map / Load Map, editor Load Map, tutorial-builder Load Tutorial) has two bodies, chosen per-open by `ShowSlots`'s optional `thumbnailStore`. **Text-only** (no store): a small fixed centered modal of click-to-load buttons. **Preview** (hosts pass `_saveStore`): a `LandscapeMenuChrome` fill-to-cap surface — a selectable slot list beside one large `MapThumbnailView`, plus Cancel / Load. In both bodies the rows are `MouseFilter.Ignore` and pointer input belongs to their container, a `RowList` (`Pass`, no `AcceptEvent`) that resolves a tap via `TapSlopDetector` and the rows' own rects — a drag therefore reaches the `ScrollContainer` and pans the list. `ToggleMode` + a `ButtonGroup` survive only as the highlight, which taps and arrow keys both set through `SetPressedNoSignal`. The `previewMaps` flag picks the loader: `RequestMap` (the `LoadStartingMap` user-then-bundled resolution) else `RequestSlot` (`user://saves/`). Distinct portrait (list-above-preview) and landscape (list-rail | preview) layouts, rebuilt on orientation flip, capped `520×920` / `920×520`. Selecting re-points the single preview; render deferred one frame. A missing/corrupt save degrades to a blank preview (row stays loadable) via `MapThumbnailView`'s log-and-bail.

  **Keyboard.** Up / Down move the selection (`ListNavMath.Step`, ends are walls) and scroll it into view (`ListNavMath.ScrollToReveal`); Enter loads the selection; Escape cancels (and dismisses the error overlay first). Both bodies carry the selection — text-only rows are toggle buttons in a `ButtonGroup` for the highlight, and a click on one still loads immediately. Navigation is handled in `_Input`, **not** `_UnhandledInput`: Godot's GUI layer runs between the two and binds Up / Down to focus navigation, so a focusable Control behind the modal consumes the key-down and only the release reaches unhandled input. Every row button sets `FocusMode = None` for the same reason. A held arrow repeats on `KeyRepeater`'s schedule (250 ms, then 50 ms a row) driven from `_Process` — platform key echoes are swallowed, since their delay is a user setting and reads as an unresponsive list. Moving the selection does not render: a thumbnail deserializes the save and rebuilds the whole map, so `_Process` fires the preview once the selection has been still 120 ms. Traces key delivery, the highlight read back off the buttons, and reveal geometry under `Input:Trace`/`Input:Debug`; `MapThumbnailView` logs requests, snapshots, and both abandon paths under `Display:Debug`.
        ...
        OnTileClickedBody(tile)  — in-grid click
          ...
          └─ TryHandlePendingUnitAction (buy/move) or TryHandlePendingTowerAction
                ...
                └─ reachable ? return true            // STAY in mode
                             : CancelModeAndConsumeUnlessOwnRedirect(coord)
                                 ├─ clicked another own territory → return false  // fall through: redirect selection
                                 └─ else → return true             // consumed: selection kept
        OnOffGridClickedBody(coord)  — water / off-grid click
          ├─ session.Mode != None
          └─ EmitRejection(level, coord) → CancelPendingAction() → return  // selection kept
...
GameController.OnEndTurnPressed
  ...
  │     └─ neutral seat only: stamp Vikings.LastCompletedRound; total wipeout → Vikings win
...
- **`ComputerAi`** — the only AI (drives every `PlayerKind.Computer` slot).
  ...
  **Phases 1, 2a, 2b, and 3 take their best legal candidate regardless of
  delta sign** (`BestDeltaAboveThreshold` with `threshold = int.MinValue`) — an
  ...
  `BestDeltaAboveThreshold` logs one `[candidate]` verdict per scored candidate
...
- **`AiStateScorer`** — pure `GameState → int` (self value minus enemy values).
  ...
  **Passive barbarians** cost per-action to provoke: `BarbarianProvokePenalty`
  (destination in a non-aggro barbarian territory ⇒ `BarbarianProvokeWeight` 2
  × the summed `UnitValue` of every unit that would wake, `[barb-avoid]` at
  Debug) is *subtracted* from move and buy-unit candidate deltas in
...
- **`AiActionLowering`** (Controller) — `GameController` wraps every injected
  ...
  bypass it (`VikingAi`; they never build) — the neutral seat's analogous
  per-turn decision state is the passive-wander guard set the driver owns
  (`AiTurnDriver._vikingWandered`; see **Barbarian aggro**).
...
- **Replay version.** A save with a `Replay` block carries `ReplayVersion`
  ...
  beats (PRNG swap, capital/tide selection rules, movement legality,
  purchase-price retunes). On load a
...
A local, platform-independent achievement record with a main-menu browse panel. The local record is the **authoritative source**; no platform service is involved, and none is required. Spans all four layers, one-way:

- **Model (Godot-free, unit-tested):**
  - `AchievementRecord` (`src/FourExHex.Model/AchievementRecord.cs`) — one entry per id: `Order` (1-based unlock sequence, 0 = locked) and `Progress` (best value ever reached). `Unlock` / `SetProgress` return "changed" so the caller decides whether to hit disk; unlocks are append-only and progress only rises. **The record never filters against the catalog** — whatever ids it read are re-emitted on save, so a build that knows fewer achievements cannot destroy a record written by one that knows more. `FromEntries` is tolerant like `CampaignProgress.FromStatuses`: blank ids skipped, negatives clamped, duplicates collapsed (progress max, order lower-non-zero), unlock order renumbered 1..N.
  - `AchievementSerializer` + `AchievementData` / `AchievementEntryData` — JSON `{ FormatVersion, Entries[] }`, registered on `FourExHexJsonContext` for iOS AOT. Unknown versions throw (store catches → fresh record).
  - `AchievementRenames` — old-id → current-id map applied on load, **once, at depth 1**; a chain must be collapsed into a single entry, and a test fails the build if any value is also a key. Ids stay renameable until they are registered in a platform console against live users; after that the platform holds a record that cannot be migrated.
  - `RunStats` (`src/FourExHex.Model/RunStats.cs`) — per-player observation-only counters for the current game (`UnitsLostToBankruptcy`, `TowersBuilt`, `VikingKills`, `MaxUnitLevelFielded`), owned by `GameState.Stats`. Incremented from **live execution paths only** — `GameOperations.RecordAftermathStats` (viking kills, max level fielded), `UpkeepRules.ApplyUpkeep`'s bankrupt branch, and the two tower-build executors — never from `AiSimulator` lookahead. Captured (deep copy) in every `UndoEntry` so an undone tower/loss un-counts; persisted as the save's `RunStats` block (omitted when all-zero, so fresh games and pre-v21 saves keep their wire format); zeroed by `ReplayRecorder.BeginReplay` because playback re-executes real beats; **excluded from `GameStateChecksum`** so determinism goldens never depend on it.
- **Controller (Godot-free, unit-tested):**
  - `AchievementEvent` — an abstract record, not an enum. `GameEndEvent` carries the game-end facts (`HumanWon`, `Mode`, `WinnerDifficulty`, `WonByClaim`, `TurnNumber`, `LandTilesRemaining`, winner-scoped `WinnerUnitsLostToBankruptcy`/`WinnerTowersBuilt`, device-wide `VikingKills`/`MaxHumanUnitLevel`); `CampaignLevelWonEvent` carries a newly-won campaign level plus the ladder counts (`WonCount`, `TierIndex`, `TierWonCount`).
  - `AchievementCatalog` — the enumerable definition table (21 entries across `Victory`/`Campaign`/`Modes`/`Skill`), and the single list a platform-console registration would derive from. `Target` derives the kind: 1 is boolean, above 1 a counter (mapping to Game Center percent and Play Games steps). Each row carries its own `Advance(AchievementEvent) → int` predicate over the facts records, so adding an achievement is one table entry — and never a new raise site unless it needs a genuinely new observation. Lives here rather than Model because the display keys are `StringKeys` constants.
  - `AchievementTracker` — applies one `AchievementEvent` to the store: skip if already unlocked, else `min(stored + delta, target)`, report, and unlock on reaching target. Returns the newly unlocked ids. A delta above 1 is how per-game counters accumulate (Viking Slayer adds the whole game's `VikingKills` at once).
  - `AchievementPanelLayout` — partitions the catalog into titled sections (categories in enum display order, rows in catalog order) so the panel's grouping is unit-tested.
  - `IAchievementStore` / `NullAchievementStore` — the store boundary and **the first-party seam**. `ReportProgress(id, current, target)` carries the target because that is what both platforms need; `Unlock` is append-only because neither can revoke. The null implementation is what a `GameController` gets by default, keeping the playtest harness, tutorial builder, and the test suite structurally inert.
  - `AchievementDisplay` — masks a hidden achievement's title/description keys until earned. `AchievementBannerContent` — unlock banner copy, returning null for an id this build doesn't recognize.
- **Scripts (Godot view layer, test-excluded):**
  - `AchievementStore` (`scripts/AchievementStore.cs`) — static persistence to the `user://achievements.json` **sidecar**, so deleting saves never costs an earned achievement. Mirrors `CampaignStore`: lazy load with a fresh-record-first poison guard, write-through per change via `AtomicUserFile`, `GD.PushWarning` + fresh fallback on corruption. `GodotAchievementStore` adapts it to `IAchievementStore`.
  - `AchievementsPanel` (`scripts/AchievementsPanel.cs`) — a `CanvasLayer` modal on the `CreditsPanel` skeleton, reached from the main menu's **Achievements** button. Renders `AchievementPanelLayout.Groups()` as category sections (name over a gold rule) on open. Earned rows ink gold; counters show `current / target`.
  - `AchievementToastLayer` (`scripts/AchievementToastLayer.cs`) — the unlock toast surface: an **autoload `CanvasLayer`** (drawn above the modal layers), so toasts still queued when a game scene is torn down keep delivering over the next scene. Gold border, trophy glyph, pinned top-center one corner-zone pad (`UiMetrics.CornerZoneEdgePadPx`, the pad `HudBars` gives the HUD corner chips) below `SafeArea.Current.Top`, so it rides as high as the OS notch allows and simply draws over whatever chrome is beneath it; the landscape width clamp reserves both rails. `PositionBanner` re-runs on resize, `SafeArea.Changed`, and per toast (orientation can change mid-drain); it measures the copy at the label's wrap width and grows the panel past its 64 px design height via `HudPanelMath.FitHeight` when a long title wraps on a narrow portrait viewport (the trophy glyph is anchored to the full height, so it stays centered). Logs `[banner] placed` with the computed top, inset, and viewport, and `[banner] fit lines=… height=…`. Simultaneous unlocks queue FIFO and play one after another (`AchievementToastQueue`, Controller, unit-tested — the layer owns only the chrome and tween); a toast with more waiting behind it uses a shortened hold so a burst reads brisk, and the last keeps the full hold. Flushes itself on recording chrome. `HudView.ShowAchievementBanner` is a forwarding stub (the `IHudView` seam is unchanged, so mocks and headless views still capture per-call).

**Award call path.** `GameController` owns the funnel: `GameOperations.CheckGameEndConditions` → `HandleGameEnded` → `AwardEndOfGameAchievements` → `GameEnded?.Invoke()`, so subscribers (the campaign result recorder, the trailing `RefreshViews`) see the updated record in the same frame. The award assembles one `GameEndEvent` on **every** untainted game end — win, loss, or stasis — so mechanic milestones (Chain of Command) unlock from a loss; `HumanWon` is true only when `SessionState.Winner` resolves to a `PlayerKind.Human` seat (a stasis end and a `PlayerId.None` winner — Viking wipeout — are both "nobody won"). `SessionState.WonByClaim` (set by `GameOperations.DeclareWinner(…, byClaim: true)` at the two claim-victory sites) is the only win-kind distinction: every non-claimed win is the elimination family. Any human seat counts; the record belongs to the device. **Campaign seam:** `Main.OnGameEndedRecordCampaignResult` (a `GameEnded` subscriber, so it runs after the game-end award) calls `GameController.RaiseCampaignLevelWon` when `CampaignStore.MarkWon` reports a newly-won level — the seam re-checks `AwardsEnabled` and shares the unlock→banner loop, and the newly-won signal is idempotent by construction so a replayed ending cannot re-fire it.

**Why the guard is load-bearing.** `ReplayRecorder.BeginReplay` clears `GameEndedFired` and `SessionState.Winner` and drives the recorded beats to the same ending, so the game-end funnel fires again on **every** playback. An append-only unlock survives that; a counter would not. `AwardsEnabled` excludes replay, preview (authored beats), and recording mode (which forces an all-human roster, so the winner-is-human test alone would not exclude it), and a one-shot per-game latch backs it up. Diagnostic runs need no clause: `Main` constructs the real store only outside diagnostic mode, so a `FOUREXHEX_6AI` session holds a `NullAchievementStore` and cannot write at all.

**No taint bit.** The `#if DEBUG` CheatMenu's one state-mutating entry, Reset Achievements, clears the record rather than granting anything, so it can only cost a player unlocks — never manufacture them. A "run was assisted" flag belongs on the day a cheat can influence a game's outcome.
...
A thin input listener (`_Input`, not `_UnhandledInput`, so the summon gesture wins over focused Controls) owning a private `EscMenu`. Entries: **Tutorial Builder** (sets `TutorialBuilderRequest.ConfigRequested` and routes to the main menu, which opens the shared player-setup page with the `TutorialBuilder` purpose — see *Tutorial builder*), **Toggle Recording Mode On/Off** (label names the action for the current state), **Demo Replays** (re-`Show`s the same `EscMenu` with one option per bundled `demo_*` tutorial plus Back — the rebuild-in-place submenu pattern; picking one sets `DemoReplayScene.PendingName` and changes scene — see *Demo replays* below), **Determinism Check** (runs `DeterminismProbe.Run` inline with the Headless views and shows the digest triple in a `ModalChrome` overlay, plus one grep-able `[determinism-probe]` `GD.Print` line — the on-device equivalent of the `FOUREXHEX_6AI_QUICK` digest on targets where env vars can't be set), **Reset Achievements** (`AchievementStore.Reset` behind a `ConfirmModal` the menu owns alongside its `EscMenu` — the record is the one player state with no second copy; awards fire only on a human win and never re-fire, so wiping it is what makes toast and award work re-testable), **Close**. Adding a cheat = adding an `EscMenu.Option` in `Toggle`. Instrumented under `Log.LogCategory.Cheat`.

### Demo replays (`DemoReplayScene`)

`scripts/DemoReplayScene.cs` (root of `res://scenes/demo_replay.tscn`, reached only from the cheat menu's Demo Replays picker) plays a bundled demo hands-free with Recording Mode active — the promotional-capture player. Enumeration: `SaveStore.ListBundledDemoNames()` scans `res://tutorials/` and filters through `DemoCatalog.FilterDemoNames` (Model, unit-tested): the `demo_*.json` subset, sorted, extension stripped. Handoff is the `LoadRequest` idiom — the picker sets the static `DemoReplayScene.PendingName`, the scene reads and clears it in `_Ready`.

The scene plays **any save carrying a replay** — a Tutorial-Builder recording or a saved real game alike (both surface it as `LoadedSave.Replay`, the accessor `TutorialReplayFidelityTests` drains). Flow: `MapEditorPanel` (painting off) → `LoadFromMap` → roster → `ResetToTutorialStart(InitialSnapshot)` → `BuildLiveStateWith` + `PreviewSetup.Apply` → real `HudView` → replay-only `GameController` (`previewMode`, no-op `aiChooser` so Computer slots idle out the end-hold, pinned paced via `replayIsInstantMode: () => false`, `replayFastForwardsIdleTurns: true`) → `BeginReplay()`. The roster is the **save's own player list** so the replay's `Actor` indexes stay aligned with the recorded `TurnState`: a single-human roster keeps its recorded kinds (fog follows the recorded player), an all-Human builder roster is re-kinded player-0-Human/rest-Computer, since `VisibilityRules.BuildProjection` only projects fog for exactly one human. Recording Mode is forced on at entry if off, and restored on exit iff this scene turned it on. `ReplayEnded` → 1.5 s final-frame hold → single guarded `ExitToMenu()` (also the ESC-menu Main Menu action): restores Recording Mode, `AbandonGame`, back to the main menu. A `ReplayDemoStartBeat` in the recording is honored as everywhere: playback fast-forwards silently to the marker.

**Camera follow.** The scene subscribes `GameController.ReplayBeatPreviewing` and pans via `HexMapView.CenterOnCoordIfOffscreen(coord)`: `ReplayFocus.FocusCoord(beat)` (Model, unit-tested) names the tile where the beat's visible effect lands (move/buy/tower destination, rally target, viking landing; null for locationless beats), and `CameraFocusMath.IsOutsideComfortZone` (ViewMath, unit-tested) keeps the camera still while actions land inside the central 60% of the inset-adjusted play area — outside it, the existing `CenterOnCoord` ease pans over, zoom untouched.
...
- **Palette.** `MapEditorHudView` builds `HexPaletteButton` swatches: one per player color, a **neutral (unowned land)** swatch, plus water, tree, capital, tower, gold, mountain and four **starting-unit** toggles (one per `UnitLevel`). The selected index is read by `OnCoordClicked` / `ApplyPaintAt` and dispatched to one of `MapEditPaint`'s pure functions (`PaintLand`, `PaintNeutral`, `PaintCapital`, `PaintTowerToggle`, `PaintTreeToggle`, `PaintWater`, `PaintGoldToggle`, `PaintMountainToggle`, `PaintUnitToggle`). Each mutates the grid in place, then re-runs `TerritoryFinder` + `CapitalReconciler` (except `PaintCapital`, which honors the user's exact pick, and the gold/mountain flag toggles, which leave the partition untouched).
- **Starting units.** `MapEditPaint.PaintUnitToggle` seeds a garrison. The unit's owner comes from the tile, never chosen separately, so a neutral tile yields a `PlayerId.None` unit — a viking raider. Same level already on the tile → cleared (the toggle); any other unit, `Tree` or `Grave` → replaced. It refuses out-of-bounds, water, `Capital` and `Tower` tiles (both inviolable to every place-a-unit path, `MovementRules.ValidTargets` included), a **non-neutral singleton** (a one-tile territory has no capital and so no treasury, so `UpkeepRules.ApplyUpkeep` would grave the unit on its first upkeep tick — neutral singletons are fine, vikings being upkeep-exempt), and `Commander` on neutral (viking waves are never Commander, so no level-4 raider exists). `ReconcileUnits`, run from the shared `Reconcile` after every paint, keeps ownership derived from the territory continuously: a repaint that changes a garrisoned tile's owner re-owns the unit — including onto neutral, where it becomes a raider — and removal is reserved for states with no valid unit (a non-neutral singleton; a `Commander` turned neutral). Units on vanished tiles need no handling, the tile carrying the occupant away with it. Placed units carry no upkeep through round 1 (see *Turn structure*) and round-trip through `EditorSnapshot` and `SerializeMap` unchanged. Traced under `Log.LogCategory.LevelDesign` (`[level] unit place/clear/reject/re-own/remove`), so the Godot editor and the headless harness share one trace.
- **Neutral hexes.** A neutral hex is land owned by `PlayerId.None` — capturable by any adjacent player like enemy territory (`tile.Owner != attackerTerritory.Owner` is the predicate); once captured it's ordinary owned land. Authored two ways: the map editor's `PaintNeutral` brush, and procedurally — `MapGenerator` produces neutral land via the mountain/gold scatter and the `MapGenOptions.NeutralDensity` plain-neutral blob. `PaintNeutral` sets owner to `None` and clears the one occupant neutral land can't hold, a `Capital`; owner-agnostic `Tower`/`Tree`/`Grave` survive, and a `Unit` is re-owned into a raider by `ReconcileUnits`. Neutral tiles flood-fill into a `None`-owned `Territory` that generates no income (`Treasury.CollectIncomeFor` skips non-owned capital-less territories) and never gets a capital — `CapitalReconciler.Reconcile` short-circuits a `None`-owned territory to capital-less and throws if a `Capital` is found there. Neutral capture logs under `Log.LogCategory.Capture` (`[capture] neutral hex {coord} -> {player}` from `GameOperations.HandleCapture`). Save/load round-trips neutral tiles (`None` encodes as wire index `-1`).
- **Responsive land swatches.** The land-color swatches plus the neutral swatch (the "owner" group) collapse to a single cycling `HexPaletteButton` when the viewport is narrow. The full `_landRow` and the lone `_landCycleButton` live side-by-side; `OnViewportMetricsChanged` (from `OrientationHud`) toggles visibility by width threshold (`FullLandRowWidth{Portrait,Landscape}`). The collapsed button is select-first-then-cycle: when land isn't active a press selects it at `_lastLandPaletteIndex`; once active each press advances through the player colors then the neutral slot, wrapping. Its `FillColor` (neutral shows `PlayerPalette.Neutral` gray) and selection outline track state via `RefreshLandCycleVisual`.
- **Responsive unit swatches.** The four starting-unit swatches collapse the same way, mirroring the play HUD's buy row (four buttons where there is room, one cycling button whose glyph *is* the armed level otherwise — `HexPaletteIcon.Unit` + `HexPaletteButton.Level`, drawn by `HudIcons.DrawUnit`). They expand only in the portrait bottom bar at a non-compact width (`ExpandedUnitPalette`): the 78-px landscape rail already carries the land swatches plus the paint tools in one column, so it takes the cycle button at every width. `OnUnitCyclePressed` is select-first-then-cycle like the land button, wrapping Recruit → Commander; `RefreshUnitCycleVisual` tracks the glyph and outline.
- **Save format.** Editor maps written with `SaveSerializer.SerializeMap`: a turn-0 `TurnState` (the on-disk "starting map" marker) serialized with the full 6-slot bake roster (kinds + difficulties, incl. `None`). At play time `Main` detects `TurnNumber == 0` to branch into the "starting map" flow: the baked roster when present (`LoadedSave.MapHasBakedKinds`; `None` slots are dropped on deserialize), else the legacy slot-0-Human default — plus a fresh `TurnState`, empty `Treasury`, and the saved grid + territories + water + mode + pre-placed trees/towers/capitals/units.

## Level-design harness

Headless authoring + playtesting of starting maps — the same artifacts the map editor produces, with no Godot on the path. Runbook: `LEVEL_DESIGN.md`. Three pieces:

- **`LevelWorkspace`** (Model) — the map editor's draft state as a plain object: blank all-water or `MapGenerator`-seeded init, paint ops dispatching to `MapEditPaint` (threading the returned territory list — the capital-inheritance invariant), `PlaceUnit(level, coord)` for starting garrisons, a 6-slot kinds/difficulties roster (painting land for a dormant slot auto-activates it as Computer/Soldier; explicit `SetSlot` choices are never overridden), `Validate()` → `MapRosterRules.ValidateForSave`, and `ToJson`/`FromJson` round-tripping the exact `SerializeMap` starting-map format (`FromJson` infers Cols×Rows from the tile + water offsets). `MapTextRenderer` renders grid + water as odd-r ASCII (owner slot digit + occupant/feature mark per cell) with per-slot tile/capital counts.
- **`LevelPlaytest`** (Controller) — plays a map JSON N times as all-AI games in-process: per game it re-deserializes the JSON (fresh mutable state), rebuilds the fresh-game `GameState` exactly like `Main`'s starting-map branch, forces every rostered slot to `PlayerKind.Computer` (baked difficulty kept — the campaign-sweep substitution), and runs one synchronous `StartGame()` (the `DeterminismProbe` pattern). A recording `IHudView` samples per-turn land counts + eliminations; `PlaytestMetrics` derives integer-only fun proxies (lead changes, decided turn, closeness %, elimination turns) aggregated by `PlaytestReport.Format()`. Same map + seeds → byte-identical report.
- **`tools/FourExHex.LevelDesigner/`** — the CLI shell (`new` / `show` / `edit` / `roster` / `validate` / `playtest`; `edit` ops include `unit LEVEL C,R...`, whose level arg parses like `land`'s slot), defaulting to the game's `user://maps/` directory so output appears in Load Starting Map and the in-game editor immediately.
- **Shipped set** — maps committed to the repo `maps/` dir (= `res://maps/`) and named in `StartingMapCatalog.Names` (Model) ship in the build: `SaveStore.ListStartingMaps()` merges them after the user's maps (same-named user maps shadow them; rows carry `SaveSlotInfo.IsBundled`, shown as "built-in" in the picker), and `LoadStartingMap` resolves user → `res://maps/` → `res://tutorials/`. A hardcoded catalog rather than a `res://` scan — PCK directory listing is unreliable; reading known paths is not. All four export presets include `maps/*.json`.

Instrumented under `Log.LogCategory.LevelDesign` (`Debug` per paint op / validation, `Info` per playtest game).

## Tutorial builder

`TutorialBuilderScene` (root of `res://scenes/tutorial_builder.tscn`, reached from the cheat menu via the main menu's shared player-setup page — purpose `TutorialBuilder`) is a 3-mode authoring tool. Tutorials are v4 save files in `user://tutorials/` carrying both a `Tutorial { Title }` block and a `Replay { InitialState, Beats }` block — the same Replay format every in-progress save ships. The scene reuses the Map Editor body: a single `MapEditorPanel` built in `_Ready`, never torn down. Mode switching only flips `panel.PaintingEnabled` and per-mode chrome `Visible`, so the painted draft survives transitions.

**Entry config.** `_Ready` reads `TutorialBuilderRequest.Pending { Kinds, Mode }` (a direct scene launch falls back to 6-color Freeform): the roster is `MapRosterRules.PreviewRosterFromKinds(kinds)` (Model, unit-tested; the active non-`None` colors forced Human — `MapEditorScene` derives its preview roster through the same helper), the Map Edit palette is gated with `MapEditorHudView.ApplyRosterKinds(kinds)`, and the mode lands on `MapEditorPanel.LiveMode` — consumed by `BuildLiveStateWith` (so Record/Preview run the mode's mechanics) and by the Save path (`BuildSaveState(LiveMode)` bakes it), and refreshed from the file by `LoadFromMap` so a loaded tutorial re-records and previews under its recorded mode.
...
- **Record** — `panel.PaintingEnabled = false`; `RecordPane` builds a transient `GameController` over the draft with every roster slot forced `PlayerKind.Human`. Its `HudView` occupies y=0..60. Dev plays hot-seat; the recording pipeline (`_replayBeats` via `TrackHandler` / `AiTurnDriver.ApplyAiActionCore`) captures game-action beats automatically — including rejected unit-move attempts, auto-captured as `ReplayRejectedMoveBeat`s (recording mode only). Three HUD authoring buttons (hidden outside Record; revealed via `SetAdd*ButtonVisible`) author the other tutorial-only beats inline: **`+ Text`** (narration dialog → `ReplayDisplayTextBeat`), **`◎`** (current selection → `ReplaySelectTerritoryBeat` anchored at its capital), **`»`** (`ReplayDemoStartBeat` fast-forward marker). Fog Of War's undo/redo lock is waived for authoring: the controller's fog gates exempt `recordingMode` (the author must be able to revert mistakes; playback re-fogs from scratch) and the pane calls `HudView.SetFogUndoExempt(true)` so the buttons match — pinned by `FogOfWar_RecordingMode_UndoRedoAllowed_ForAuthoring`.
...
5. `GameController` built with:
   ...
   - `onAfterRefresh: () => { replayAi.ConsumeElapsedNeutralSeatBeats(state); narration.Tick(); cues.Apply(); }` (elapsed neutral-seat beats clear first so the beat behind them presents on the same refresh; the narration driver then sets its `IsPresenting` flag before cues check it)
...
- **`ReplayDrivenAi.ConsumeElapsedNeutralSeatBeats`** advances past beats the neutral seat owns, once that seat's turn has passed. The seat plays live in Preview — `AiTurnDriver` routes it to `VikingAi`, not the scripted chooser — so its recorded beats have no other consumer, and a parked cursor rejects every player input as a desync. A beat belongs to the seat when its `Actor` is at or past the play roster's size: the seat always sits last, and a recording made against the builder's six slots stamps a higher index than the play roster trimmed to land-owning colors (`PlayTutorialScene`). Beats owned by a live player are never skipped, so a genuine desync still surfaces.
...
**`InstructionDemoView`** (`scripts/InstructionDemoView.cs`) — the graphics sub-panel: a complete second (model, controller, view) stack playing a recorded tutorial hands-free, on a loop, with the recorder's turn-end fast-forward on (`replayFastForwardsIdleTurns: true`) so idle opponent turns flick past.
...
**Content pipeline.** Each page's demo is authored in the tutorial builder (small painted map + recorded beats, incl. the tutorial-only kinds above), saved to `user://tutorials/`, and bundled by copying the JSON into `res://tutorials/` as `instr_<page>.json` (`instr_territories`, `instr_recruit`, `instr_defense`, `instr_towers`, `instr_commander`, `instr_income`, `instr_trees`, `instr_winning`). Page copy lives in the string store (`hud.instructions.page.*`). The `export_presets.cfg` `tutorials/*.json` include filter ships them — the same filter bakes the `demo_*` Demo Replays set (see *Demo replays*), which may be Tutorial-Builder recordings or saved real games; `TutorialReplayFidelityTests` auto-discovers every `tutorials/*.json` and proves each drains to completion.
...
Tile fills stay one `Polygon2D` each (recolored, not recreated, on capture). Remaining per-capture cost: `RefreshOccupantVisuals` recreating occupant nodes each refresh. Diagnostics behind the `[hitch]` prefix: `Log.Since` rides the ordinary `[Conditional("FOUREXHEX_LOGGING")]` strip, while `LogLongFrame` (CPU/draw split in `_Process`) and the one-shot `DumpSceneComposition` stay `[Conditional("DEBUG")]` — they poll `Performance` monitors per frame and walk the scene tree, so they are dev-only.
...
| `TopRightZone` | content-sized HBox anchored top-right | both | help / options |
| `BottomBar` | full-width Panel anchored bottom | portrait only | Action button rows |
| `LeftRail` (+ `LeftRailGroup` inner VBox) | `HudBars.RailWidth` (gutter + button + gutter = 84 px) Panel anchored left, full height, at `HudCornerLayout.RailSideOffset` | landscape only | Create / paint cluster |
...
**Corner zones can collide, and the layout pass won't say so.** Each corner zone is a content-sized anchor point growing toward the middle — no width budget, no shrink, no clipping — and neither knows the other exists, so on a narrow viewport they pass through each other and `TopRightZone` (added last) paints over the chips. `ApplyLayout` therefore defers `OrientationHud.LogCornerFit`, which flattens both zones to their visible blocks (descending through bare box containers, so a styled chip or button is the unit that can collide), and under `Render`: `Log.Warn`s `[corner-fit] OVERLAP <leftBlock>×<rightBlock> by Npx` on any block-rect intersection, else `Debug`s the widest-block budget gap from `HudCornerLayout.CornerGap`. Summed zone widths are NOT the test — portrait stacks the chips vertically, so the wide `_goldChip` clears the chrome buttons by sitting below them. Both orientations keep `_undoCluster` out of the top corners — down with the other action buttons (portrait bottom bar row 1; landscape pins it as its own bottom-left strip, mirroring the End Turn + Automate bottom-right strip via `PinBottomCorner`) — because four chrome buttons plus the chips genuinely do not fit a phone's width; `tests/HudCornerLayoutTests.cs` pins that budget against the real chip/button widths (`UiMetrics.GoldLabelMinWidthPx`, `TouchButtonSizePx`, `ChipPaddingPx`, …) at the narrowest supported viewports. HUD blocks carry explicit `Name`s (`StatusChip`, `GoldChip`, `HelpButton`, `OptionsButton`, `UndoButton`, `RedoButton`) so the warning names a culprit instead of `@PanelContainer@400`.

**The landscape bottom-left stack is guarded the same way.** Bottom-up: the seed / map-name label sits in the very corner (the same spot as portrait), the undo/redo strip is lifted above it by `HudBottomStripLayout.StripBottomOffset` (the End Turn + Automate strip takes the same lift so the two sides stay level), and in expanded landscape both rail groups back their bottom edge off by `RailClearance` so the last rail button clears the strip. The label grows upward (`GrowVertical = Begin`) from a measured `SeedLabelHeight()`. `HudView.OnLayoutApplied` defers `LogSeedLabelFit`, which under `Render` `Log.Warn`s `[seed-fit] OVERLAP <block> by Npx` when the label and `UndoCluster`, or `UndoCluster` and the left rail's last block, intersect, else `Debug`s `[seed-fit] clear — gapToRail=… gapToStrip=…`; `tests/HudBottomStripLayoutTests.cs` pins the three distances.
...
- *Corner zones* (status/gold chips, help, Options) + the pinned bottom-right strip use no horizontal safe inset — claiming the corner real estate rails leave. On iPhone landscape corner chrome may visually overlap the notch, but iOS routes taps through.
...
| Portrait TopRight | `_helpButton` + `_optionsButton` | Same |
| Portrait BottomBar | Row 1 (space-between): nav cluster left, `_undoCluster` right. Row 2 (space-between): `_actionCluster` (buy cycle + Build Tower) left, `_endTurnButton` right | Row 1 same; Row 2 buy palette → 1×4 radio |
| Landscape TopLeft | `_statusChip` (1-swatch) + `_goldChip` inline | Same, expanded swatches |
| Landscape TopRight | `_helpButton` + `_optionsButton` | Same |
...
| Landscape bottom-right strip | `_endTurnButton` at the corner, then `_automateButton`, then `_undoCluster`, laid right-to-left by `PinBottomRightStrip` (anchored directly to `HudView`, outside rails) | Same; right rail group pushed up by `endTurnClearance = 88px` |
...
- **Two gates.** (1) Compile-time: `Log.Trace` / `Debug` / `Info` / `Since` are `[Conditional("FOUREXHEX_LOGGING")]`, so without that constant the compiler removes the call and its argument evaluation; `Log.Warn` / `Error` carry no attribute at all. Every project holding call sites (`FourExHex.Model`, `FourExHex.Controller`, `FourExHex` itself, `tests`) defines the constant in **every** configuration, shipping included — a release build's log is the only diagnostic a player's bug report can carry, so dropping the define is the deliberate kill switch rather than the default. The constant is pointedly not `DEBUG`, which also gates the `#if DEBUG` CheatMenu hooks; tying them together would mean shipping a cheat menu to get a log. (2) Runtime: each `Log.LogCategory` (`Ai`, `Turn`, `Capture`, `Tutorial`, `Render`, `Anim`, `Input`, `Display`, `Hud`, `Undo`, `Cheat`, `Campaign`, `MapGen`, `Replay`, `Tide`, `Fog`, `Tree`, `Automate`, `Viking`, `Determinism`, `LevelDesign`, `Share`, `Report`, `Achieve`) has an independent minimum `Log.LogLevel`; a message emits only if its level ≥ the category threshold.
- **Editor runs are silent; exported builds are verbose.** Every category defaults to `Off`, so a dev launch prints nothing until `FOUREXHEX_LOG` or `SetLevel` raises one. An exported build has no way to set that env var, so `LogBootstrap` puts every category at `Debug` when `LogDefaults.ShouldDefaultVerbose(spec, OS.HasFeature("template"))` says so — an explicit spec always wins, and `template` is false under the editor binary, which is what keeps dev play and the seeded `FOUREXHEX_6AI*` determinism diff quiet.
- **`application/run/flush_stdout_on_print=true`** (`project.godot`). Godot defaults it false in release, which leaves the tail of `user://logs/godot.log` sitting in a buffer — precisely the moments before a player files a report. Flushing per line costs nothing at the measured rates and makes the attached log current.
...
- **Pre-computing helpers** (`GameController.LogTurnStart`, `LogAction`, `LogGameEndDiagnostics`, `LogCaptureDiff`) build their own summary data before logging, so each carries its own `[Conditional("FOUREXHEX_LOGGING")]` — they ship with the rest of the log, since the turn / AI-action / capture trail is the substance of a bug report. `HexMapView.LogLongFrame` and `DumpSceneComposition` stay `[Conditional("DEBUG")]`: they poll per-frame `Performance` monitors and walk the scene tree, making them dev profiling tools rather than field diagnostics. `Warn`/`Error` sites keep their precompute.
...
├─ TutorialBuilderRequest.cs ─ static cheat-menu→menu→builder handoff
│                           (ConfigRequested flag; Kinds + Mode request)
├─ PlayTutorialScene.cs   ─ "Play Tutorial" scene root; hosts MapEditorPanel +
│                           PreviewPane + EscMenu, loads + plays bundled
│                           full_tutorial
├─ DemoReplayScene.cs     ─ Demo Replays scene root: hands-free replay of a
│                           bundled demo_* save in Recording Mode, camera
│                           follow + turn-end fast-forward, exit to menu
...
│                           "GameController ↔ AiTurnDriver split"
├─ StepPacing.cs          ─ shared step-beat cadence: the delay constants
...
│                           [Conditional("FOUREXHEX_LOGGING")] strip)
├─ LogDefaults.cs         ─ boot-time verbosity decision (exported
│                           builds start verbose; editor stays silent)
│
...
├─ ReplayFocus.cs         ─ beat → effect-tile mapping for the demo camera follow
├─ DemoCatalog.cs         ─ demo_*.json name filter behind the Demo Replays picker
...
├─ Player.cs              ─ + PlayerKind {Human,Computer,None,Neutral};
│                           Player.Neutral singleton (the rotation's
│                           trailing seat); BuildRoster (skips None),
│                           BuildCampaignRoster
