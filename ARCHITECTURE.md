...
- **`LevelWorkspace`** (Model) — the map editor's draft state as a plain object: blank all-water or `MapGenerator`-seeded init, paint ops dispatching to `MapEditPaint` (threading the returned territory list — the capital-inheritance invariant), a 6-slot kinds/difficulties roster (painting land for a dormant slot auto-activates it as Computer/Soldier; explicit `SetSlot` choices are never overridden), `Validate()` → `MapRosterRules.ValidateForSave`, and `ToJson`/`FromJson` round-tripping the exact `SerializeMap` starting-map format (`FromJson` infers Cols×Rows from the tile + water offsets). `MapTextRenderer` renders grid + water as odd-r ASCII (owner slot digit + occupant/feature mark per cell) with per-slot tile/capital counts.
...
- **`tools/FourExHex.LevelDesigner/`** — the CLI shell (`new` / `show` / `edit` / `roster` / `validate` / `playtest`), defaulting to the game's `user://maps/` directory so output appears in Load Starting Map and the in-game editor immediately.
