...
- **Scripts (Godot view layer, test-excluded):**
  ...
  - `HudView.ShowAchievementBanner` — the unlock toast: gold border, trophy glyph, held longer than the transient banner. Takes the **topmost** toast slot rather than stacking below the bankruptcy and transient banners — those fire from upkeep at turn start, an unlock fires at game end, so they cannot co-occur. Simultaneous unlocks queue FIFO and play one after another (`AchievementToastQueue`, Controller, unit-tested — the HudView shell owns only the tween); a toast with more waiting behind it uses a shortened hold so a burst reads brisk. Recording chrome flushes the queue along with hiding the banner.
