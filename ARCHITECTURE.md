...
- `src/FourExHex.Model/` — pure model, rules, AI (incl. `AiDispatcher`), `UndoStack<T>` + `GameStateSnapshot`, save serialization (`SaveSerializer`, `Replay`, `ReplayBeat`, the `Tutorial` POCO), `MapGenerator` / `MapEditPaint` / `EditorSnapshot`, `LevelWorkspace` / `MapTextRenderer` / `SaveNames`, `PlayerId`.
