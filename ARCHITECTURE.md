...
**Recording mode** (`scripts/RecordingMode.cs`): a session-scoped static flag (`Active` + `Changed` event, never persisted) that hides promo-noisy chrome for clean promotional captures while the game stays fully playable. Only the cheat menu flips it, so Release builds carry the flag permanently false. Subscribers react immediately on `Changed` and re-assert per-`Refresh`: `HudView` hides the action-hint/tutorial panel, gold chip, turn + swatch chip, bankruptcy toast, and transient banner (show-sites `ShowTransientBanner` / `SummonCapitalAlertNotice` early-return while active; `ShowTutorialMessage` / `ShowTappableTutorialMessage` keep all state — `_externalMessageActive`, text, tap catcher — but leave the panel invisible, so toggling off restores an active message); `HexMapView.RedrawWarningBadges` leaves the capital warning badges cleared. Buttons and endgame overlays are untouched. View-layer only — no controller/model involvement.
