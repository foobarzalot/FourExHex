...
public partial class HudView : OrientationHud, IHudView
{
    ...
    // Lives OUTSIDE this scene-scoped view: the AchievementToastLayer
    // autoload owns the chrome, FIFO queue, and tween, so a queue still
    // draining when this HudView is freed (exit-to-menu, next campaign
    // level) keeps delivering over the next scene. This is only the
    // IHudView forwarding stub.

    /// <summary>Forward to the scene-independent toast layer, which
    /// queues FIFO and keeps draining across scene changes (#235).</summary>
    public void ShowAchievementBanner(string text) => AchievementToastLayer.Show(text);
    ...
    private void ApplyRecordingChrome()
    {
        ...
        if (hide)
        {
            ...
            // Achievement toasts: the AchievementToastLayer autoload flushes
            // itself on RecordingMode.Changed.
        }
