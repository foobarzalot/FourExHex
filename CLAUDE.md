...
- `src/FourExHex.Controller/` — the **Godot-free controller library** (`FourExHex.Controller.csproj`, plain `Microsoft.NET.Sdk`, **no GodotSharp**), which `<ProjectReference>`s **only** `FourExHex.Model` (one-way). Holds `GameController`, the UI-scoped `SessionState`/`SessionStateSnapshot`/`UndoEntry`, the `IHexMapView`/`IHudView`/`IAiPacer` view-boundary interfaces, the AI pacers (`AiPacer`/`GodotAiPacer`), and the `Tutorial/` Record/Preview scripting helpers (everything in `Tutorial/` except the `Tutorial` POCO). New testable orchestration logic goes HERE; the test project picks both libraries up automatically.
